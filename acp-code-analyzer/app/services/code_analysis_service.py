...
class CodeAnalysisService:
    ...
    async def _find_files(
        self, repo_path: Path, include_patterns: List[str], exclude_patterns: List[str]
    ) -> List[Path]:
    ...
    async def _analyze_js_file(self, content: str, file_path: Path) -> Dict[str, Any]:
        ...
        for match in re.finditer(func_pattern, content):
            ...
            analysis["functions"].append(
                {"name": func_name, "line_number": line_num, "type": "function"}
            )
        ...
        for match in re.finditer(class_pattern, content):
            ...
            line_num = content[: match.start()].count("\n") + 1
            analysis["classes"].append({"name": class_name, "line_number": line_num})
        ...
        for match in re.finditer(import_pattern, content):
            ...
            line_num = content[: match.start()].count("\n") + 1
            analysis["imports"].append({"module": module, "line_number": line_num})
    ...
    async def _analyze_sql_file(self, content: str, file_path: Path) -> Dict[str, Any]:
        ...
        for match in re.finditer(table_pattern, content, re.IGNORECASE):
            ...
            line_num = content[: match.start()].count("\n") + 1
            analysis["tables"].append({"name": table_name, "line_number": line_num})
        ...
        for match in re.finditer(view_pattern, content, re.IGNORECASE):
            ...
            line_num = content[: match.start()].count("\n") + 1
            analysis["views"].append({"name": view_name, "line_number": line_num})
    ...
    async def _extract_insights(
        self, analysis_results: List[Dict[str, Any]], repo_path: Path
    ) -> Dict[str, Any]:
        ...
        for result in analysis_results:
            ...
            insights["language_distribution"][lang] = (
                insights["language_distribution"].get(lang, 0) + 1
            )
