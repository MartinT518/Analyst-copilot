...
class SchemaAnalysisService:
    ...
    async def analyze_database_schema(
        ...
        """
        self.logger.info("Starting database schema analysis", connection_string=connection_string[:50] + "...")
        ...
        try:
            ...
            for schema_name in schemas:
                try:
                    schema_analysis = await self._analyze_schema(engine, schema_name, include_system_tables)
            ...
            final_result = {
                ...
                "statistics": {
                    ...
                    "total_objects": sum(
                        len(s.get("tables", [])) + len(s.get("views", [])) + 
                        len(s.get("functions", [])) + len(s.get("procedures", []))
        ...
        try:
            ...
            if schema_names:
                ...
                if not include_system_tables and engine.dialect.name == "postgresql":
                    ...
                    schemas = [s for s in schemas if s not in system_schemas and not s.startswith("pg_")]
    ...
    async def _analyze_postgresql_schema(
        ...
        try:
            ...
            self.logger.warning("PostgreSQL-specific analysis failed", schema=schema_name, error=str(e))
            analysis["error"] = str(e)
    ...
    async def _extract_schema_insights(
        self,
        schema_analyses: List[Dict[str, Any]],
        ...
        if all_tables:
            ...
            insights["table_statistics"] = {
                ...
                "tables_with_primary_keys": sum(1 for table in all_tables if table.get("primary_keys")),
                "tables_with_foreign_keys": sum(1 for table in all_tables if table.get("foreign_keys"))
            }
        ...
        if table_names:
            ...
            insights["naming_conventions"] = {
                ...
                "naming_consistency_score": max(snake_case_count, camel_case_count) / len(table_names)
            }
        
        # Complexity metrics
        insights["complexity_metrics"] = {
            "schema_complexity": len(schema_analyses),
            "average_tables_per_schema": total_tables / len(schema_analyses) if schema_analyses else 0,
