...
@app.command()
def logs(
    service: Optional[str] = typer.Option(None, "--service", "-s", help="Service to monitor (ingest, agents, postgres, redis)"),
...
@app.command()
def metrics(
    service: Optional[str] = typer.Option("ingest", "--service", "-s", help="Service to get metrics from"),
    metric: Optional[str] = typer.Option(None, "--metric", "-m", help="Specific metric to query"),
    duration: Optional[str] = typer.Option("5m", "--duration", "-d", help="Time range for metrics (e.g., 5m, 1h, 1d)")
):
...
def dashboard(
    ...
    duration: int = typer.Option(300, "--duration", "-d", help="How long to run dashboard in seconds")
):
        ...
        def create_dashboard() -> Layout:
            ...
            for service_name, status in [("Ingest", ingest_status), ("Agents", agents_status)]:
                ...
                services_table.add_row(
                    ...
                    str(status['requests']),
                    str(status['errors']),
                    f"{status['error_rate']:.1f}%" if isinstance(status['error_rate'], (int, float)) else str(status['error_rate'])
                )
            ...
            # Split layout
            layout.split_column(
                Layout(services_table, name="services"),
                Layout(containers_table, name="containers")
            )
...
def traces(
    ...
    duration: Optional[str] = typer.Option("1h", "--duration", "-d", help="Time range (e.g., 1h, 30m)")
):
...
def alerts(
    ...
    severity: Optional[str] = typer.Option(None, "--severity", help="Filter by severity (critical, warning, info)")
):
    """View active alerts."""
