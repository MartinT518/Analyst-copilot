...
def start(
    workflow_type: str = typer.Argument(
        ..., help="Type of workflow (clarifier, synthesizer, taskmaster, verifier)"
    ),
    input_file: Optional[str] = typer.Option(
        None, "--input", "-i", help="JSON file with input data"
    ),
    input_data: Optional[List[str]] = typer.Option(
        None, "--data", "-d", help="Input data as key=value pairs"
    ),
...
def synthesize(
    ...
    current_state: Optional[str] = typer.Option(
        None, "--current", "-c", help="Current state description"
    ),
    output_format: Optional[str] = typer.Option(
        None, "--format", "-f", help="Output format (table, json, yaml)"
    ),
...
def tasks(
    ...
    input_data = {"requirements": requirements, "priority": priority}
...
def verify(
    ...
    verification_type: Optional[str] = typer.Option(
        "general", "--type", "-t", help="Verification type"
    ),
    output_format: Optional[str] = typer.Option(
        None, "--format", "-f", help="Output format (table, json, yaml)"
    ),
    ...
    input_data = {"output": output_to_verify, "verification_type": verification_type}
