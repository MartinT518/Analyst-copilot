...
@app.command()
def dependencies(
    service: Optional[str] = typer.Option(None, "--service", "-s", help="Scan specific service (ingest, agents, cli)"),
    ...
    severity: str = typer.Option("medium", "--severity", help="Minimum severity level (low, medium, high, critical)")
):
                ...
                if output_format == 'json':
                    try:
                        ...
                        for vuln in vulnerabilities:
                            if vuln.get('severity', '').lower() in severity_levels.get(severity, []):
...
def code(
    service: Optional[str] = typer.Option(None, "--service", "-s", help="Scan specific service (ingest, agents, cli)"),
    output_format: str = typer.Option("table", "--format", "-f", help="Output format (table, json, sarif)"),
    confidence: str = typer.Option("medium", "--confidence", help="Minimum confidence level (low, medium, high)")
):
...
def _basic_secret_scan(scan_path: Path, exclude_patterns: List[str]):
    ...
    if findings:
        ...
        for finding in findings:
            print_warning(f"  {finding['file']}:{finding['line']} - {finding['type']}: {finding['match']}")
        raise typer.Exit(1)
    else:
...
@app.command()
def containers(
    image: Optional[str] = typer.Option(None, "--image", "-i", help="Scan specific container image"),
    severity: str = typer.Option("medium", "--severity", help="Minimum severity level (low, medium, high, critical)"),
    output_format: str = typer.Option("table", "--format", "-f", help="Output format (table, json)")
):
    ...
    try:
        ...
        if image:
            ...
            if result.returncode == 0:
                images_to_scan = [img.strip() for img in result.stdout.strip().split('\n') if img.strip()]
            else:
