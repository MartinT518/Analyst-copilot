...
from ..config import config_manager, CLIConfig, ServiceConfig
from ..utils import print_success, print_error, print_info, format_output
...
@app.command()
def set_output(
    format_type: str = typer.Argument(..., help="Output format (table, json, yaml)")
):
...
@app.command()
def set_verbose(
    enabled: bool = typer.Argument(..., help="Enable verbose output")
):
...
@app.command()
def set_debug(
    enabled: bool = typer.Argument(..., help="Enable debug mode")
):
    """Set debug mode."""
...
def validate():
    ...
    try:
        ...
        from ..client import IngestClient, AgentsClient, CodeAnalyzerClient
