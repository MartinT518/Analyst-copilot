...
from .common_schemas import WorkflowStatus, AgentType, Priority, WorkflowStep, WorkflowContext
...
class WorkflowRequest(BaseModel):
    ...
    clarified_requirements: Optional[Dict[str, Any]] = Field(None, description="Pre-clarified requirements")
...
class WorkflowResponse(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When workflow was created")
...
class WorkflowResults(BaseModel):
    ...
    synthesizer_output: Optional[SynthesizerOutput] = Field(None, description="Synthesizer agent results")
    taskmaster_output: Optional[TaskmasterOutput] = Field(None, description="Taskmaster agent results")
    ...
    # Timestamps
    completed_at: datetime = Field(default_factory=datetime.utcnow, description="When results were finalized")
...
class WorkflowMetrics(BaseModel):
    ...
    metrics_by_type: Dict[WorkflowType, Dict[str, Any]] = Field(
        ...
        description="Metrics broken down by workflow type"
    )
    
    # By time period
    daily_metrics: Dict[str, int] = Field(default_factory=dict, description="Daily workflow counts")
    hourly_metrics: Dict[str, int] = Field(default_factory=dict, description="Hourly workflow counts")
