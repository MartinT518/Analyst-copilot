...
class BaseAgent(ABC):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        request_id = input_data.get(
            "request_id", f"{self.agent_type}_{int(time.time())}"
        )
        ...
        try:
            ...
            self.logger.error(
                "Agent execution failed", request_id=request_id, error=error_msg
            )
    ...
    def _parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse JSON response", response=response, error=str(e)
            )
