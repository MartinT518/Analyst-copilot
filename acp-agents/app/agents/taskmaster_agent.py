...
"""
        ...
        # Format constraints
        constraints_text = (
            json.dumps(input_data.project_constraints, indent=2)
            if input_data.project_constraints
...
- timeline_estimate: Overall timeline estimate for implementation"""
    ...
    async def _process_request(self, input_data: TaskmasterInput) -> TaskmasterOutput:
        ...
        try:
            ...
            confidence_factors = {
                "requirements_clarity": self._assess_requirements_clarity(
                    input_data.to_be_document
                ),
            ...
            output = TaskmasterOutput(
                ...
                tasks=tasks,
                task_breakdown_summary=response_data.get(
                    "task_breakdown_summary", "Requirements broken down into implementable tasks"
                ),
                implementation_phases=response_data.get(
                ...
                metadata={
                    ...
                    "total_acceptance_criteria": sum(
                        len(task.acceptance_criteria) for task in tasks
                    ),
            ...
            self.logger.error(
                "Task generation failed", request_id=input_data.request_id, error=str(e)
            )
            raise
