    ...
    def _get_user_prompt(self, input_data: SynthesizerInput) -> str:
        ...
        if input_data.knowledge_context:
            ...
            for ref in input_data.knowledge_context:
                knowledge_context += f"- Source: {ref.source_type} (Score: {ref.similarity_score:.2f})\n"
                knowledge_context += f"  Content: {ref.excerpt}\n\n"
        
        scope_text = f"\n\nSCOPE BOUNDARIES:\n{input_data.scope_boundaries}" if input_data.scope_boundaries else ""
...
- risks_and_mitigation: List of risks and mitigation strategies"""
        ...
        try:
            ...
            for gap_data in response_data.get("gap_analysis", []):
                try:
                    ...
                    self.logger.warning("Failed to parse gap analysis", error=str(e), gap_data=gap_data)
                    continue
            
            # Calculate confidence based on various factors
            confidence_factors = {
                "requirements_completeness": self._assess_requirements_completeness(input_data.clarified_requirements),
            ...
            output = SynthesizerOutput(
                ...
                implementation_approach=response_data.get("implementation_approach", "Phased implementation approach recommended"),
    ...
    def _create_document(self, doc_data: Dict[str, Any], default_title: str) -> AsIsDocument:
        ...
        for i, section_data in enumerate(doc_data.get("sections", [])):
            try:
                ...
                self.logger.warning("Failed to parse document section", error=str(e), section_data=section_data)
                continue
        ...
        if "as_is" in default_title.lower() or "current" in default_title.lower():
            return AsIsDocument(
                ...
                executive_summary=doc_data.get("executive_summary", "Executive summary not provided"),
                sections=sections,
                current_state_analysis=doc_data.get("current_state_analysis", "Current state analysis not provided"),
            ...
            return ToBeDocument(
                ...
                executive_summary=doc_data.get("executive_summary", "Executive summary not provided"),
                sections=sections,
                future_state_vision=doc_data.get("future_state_vision", "Future state vision not provided"),
    ...
    def _assess_documentation_quality(self, as_is_doc: AsIsDocument, to_be_doc: ToBeDocument) -> float:
        ...
        # Check content depth
        total_content_length = sum(len(section.content) for section in as_is_doc.sections + to_be_doc.sections)
