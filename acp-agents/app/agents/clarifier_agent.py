...
Ensure all questions are specific, actionable, and focused on gathering the information needed to create a complete requirements specification."""
        ...
        try:
            ...
            for q_data in response_data.get("questions", []):
                try:
                    ...
                    self.logger.warning("Failed to parse question", error=str(e), question_data=q_data)
                    continue
            ...
            confidence_factors = {
                ...
                "questions_generated": min(1.0, len(questions) / self.settings.clarifier_max_questions),
            ...
            output = ClarifierOutput(
                ...
                questions=questions,
                analysis_summary=response_data.get("analysis_summary", "Request analyzed for clarification needs"),
            ...
            self.logger.error("Clarification failed", request_id=input_data.request_id, error=str(e))
            raise
    ...
    def _assess_request_clarity(self, request: str) -> float:
        ...
        specific_words = ["specific", "exactly", "must", "should", "will", "need", "require"]
        ...
        technical_terms = ["system", "database", "api", "interface", "integration", "workflow"]
            ...
            for q_data in response_data.get("questions", []):
                try:
                    question = ClarificationQuestion(
                        ...
                        importance=q_data.get("importance", "medium"),
                        suggested_answers=q_data.get("suggested_answers", []),
                        context=q_data.get("context", "Follow-up question based on previous answers")
                    )
