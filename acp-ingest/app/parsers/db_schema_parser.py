...
class DatabaseSchemaParser:
    ...
    def _is_connection_string(self, content: str) -> bool:
        ...
        return any(db in content.lower() for db in ["postgresql://", "mysql://", "sqlite://", "oracle://", "mssql://"])
    
    async def _parse_database_schema(self, connection_string: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _generate_schema_overview(self, schema_info: Dict[str, Any]) -> str:
        ...
        overview_parts.append(f"Database schema contains {table_count} table(s) and {view_count} view(s).")
        ...
        if relationship_count > 0:
            overview_parts.append(f"Schema defines {relationship_count} relationship(s) between tables.")
    ...
    async def _parse_schema_content(self, content: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
