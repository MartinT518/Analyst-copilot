...
class DeadLetterQueue:
    ...
    async def get_job_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_jobs = session.query(DeadLetterJob).filter(
                DeadLetterJob.status == JobStatus.PENDING
            ).count()
            retrying_jobs = session.query(DeadLetterJob).filter(
                DeadLetterJob.status == JobStatus.RETRYING
            ).count()
            dead_letter_jobs = session.query(DeadLetterJob).filter(
                DeadLetterJob.status == JobStatus.DEAD_LETTER
            ).count()
            resolved_jobs = session.query(DeadLetterJob).filter(
    ...
    async def cleanup_resolved_jobs(self, older_than_days: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = session.query(DeadLetterJob).filter(
                DeadLetterJob.status == JobStatus.RESOLVED,
