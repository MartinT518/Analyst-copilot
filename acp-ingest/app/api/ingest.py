...
from app.schemas import IngestJobResponse, IngestJobCreate, PasteRequest, JobStatusResponse
from app.services.ingest_service import IngestService
from app.services.auth_service import get_current_user
from app.utils.file_utils import save_upload_file, detect_file_type, get_file_info
from app.utils.logging_config import get_logger, get_audit_logger, get_performance_logger
...
async def upload_file(
    ...
    source_type: Optional[str] = Form(None, description="Source type (auto-detected if not provided)"),
...
async def list_jobs(
    ...
    try:
        ...
        for job in jobs:
            ...
            job_responses.append(IngestJobResponse(
                id=job.id,
                status=job.status,
                origin=job.origin,
                source_type=job.source_type,
...
async def retry_job(
    ...
    try:
        ...
        return JSONResponse(
            status_code=200,
            content={"message": "Job retry initiated", "job_id": job_id, "status": job.status}
        )
...
async def get_ingest_stats(
    ...
    try:
        ...
        source_type_stats = {}
        for source_type in ["jira_csv", "confluence_html", "confluence_xml", "pdf", "markdown", "paste"]:
