    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                return {
                    "status": "healthy",
                    "details": "Chroma connection successful"
                }
            else:
                return {
                    "status": "unhealthy",
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(
                f"{settings.LLM_ENDPOINT}/v1/models",
                headers=headers
            )
            ...
            if response.status_code == 200:
                return {
                    "status": "healthy",
                    "details": "LLM endpoint accessible"
                }
            else:
                return {
                    "status": "unhealthy",
...
async def check_embedding_endpoint() -> Dict[str, Any]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(
                f"{settings.EMBEDDING_ENDPOINT}/models",
                headers=headers
            )
            ...
            if response.status_code == 200:
                return {
                    "status": "healthy",
                    "details": "Embedding endpoint accessible"
                }
            else:
                return {
                    "status": "unhealthy",
...
async def health_check():
    ...
    try:
        ...
        services = {
            ...
            "chroma": checks[2] if not isinstance(checks[2], Exception) else {"status": "unhealthy", "error": str(checks[2])},
            "llm_endpoint": checks[3] if not isinstance(checks[3], Exception) else {"status": "unhealthy", "error": str(checks[3])},
            "embedding_endpoint": checks[4] if not isinstance(checks[4], Exception) else {"status": "unhealthy", "error": str(checks[4])},
            "disk_space": checks[5] if not isinstance(checks[5], Exception) else {"status": "unhealthy", "error": str(checks[5])}
        }
