...
def cleanup_old_jobs() -> Dict[str, Any]:
    ...
    try:
        ...
        # Get old jobs
        old_jobs = db.query(IngestJob).filter(
            ...
            try:
                ...
                if job.file_path and os.path.exists(job.file_path):
                    try:
                        ...
                        logger.warning("Failed to delete job file", file_path=job.file_path, error=str(e))
...
def reprocess_failed_jobs(max_retries: int = 3) -> Dict[str, Any]:
    ...
    try:
        ...
        failed_jobs = db.query(IngestJob).filter(
        ...
        for job in failed_jobs:
            try:
                ...
                if job.error_message and any(keyword in job.error_message.lower() for keyword in 
                    ...
                    logger.info("Job queued for reprocessing", job_id=job.id, retry_count=job.retry_count)
