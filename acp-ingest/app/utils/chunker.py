...
class TextChunker:
    ...
    def __init__(self, config: Optional[ChunkConfig] = None):
        ...
        self.abbreviations = {
            ...
            'vs', 'etc', 'ie', 'eg', 'al', 'st', 'ave', 'blvd', 'rd'
        }
    ...
    def _post_process_chunks(
        ...
        for chunk in chunks:
            ...
            if len(text) < config.min_chunk_size:
                ...
                if processed_chunks and len(processed_chunks[-1]['text']) + len(text) <= config.max_chunk_size:
                    processed_chunks[-1]['text'] += "\n\n" + text
                    processed_chunks[-1]['metadata']['chunk_size'] = len(processed_chunks[-1]['text'])
                    processed_chunks[-1]['metadata']['word_count'] = len(processed_chunks[-1]['text'].split())
                    continue
    ...
    def get_chunk_stats(self, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'chunks_with_code': sum(1 for chunk in chunks if chunk['metadata'].get('contains_code', False)),
            'chunks_with_lists': sum(1 for chunk in chunks if chunk['metadata'].get('contains_list', False)),
            'chunks_with_headings': sum(1 for chunk in chunks if chunk['metadata'].get('contains_headings', False)),
        }
