    ...
    async def export_search_results(
        ...
        try:
            ...
            if format_type == ExportFormat.CSV:
                ...
                return await self._export_search_results_markdown(results, filename, title, metadata)
...
""")
            ...
            for i, result in enumerate(results, 1):
                ...
                htmlfile.write(f"""    <div class="result">
    ...
    async def _export_audit_markdown(
        ...
        with open(file_path, 'w', encoding='utf-8') as mdfile:
            ...
            for log in audit_logs:
                ...
                mdfile.write(f"**Timestamp:** {log.created_at.strftime('%Y-%m-%d %H:%M:%S UTC') if log.created_at else 'Unknown'}\n")
                mdfile.write(f"**User ID:** {log.user_id or 'System'}\n")
                mdfile.write(f"**Resource:** {log.resource_type or 'N/A'} ({log.resource_id or 'N/A'})\n")
