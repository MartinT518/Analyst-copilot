...
class MetricsService:
    ...
    def collect_system_metrics(self, db: Session):
        ...
        try:
            # Count active jobs
            active_jobs_count = db.query(IngestJob).filter(
    ...
    def get_metrics_summary(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            active_jobs = db.query(IngestJob).filter(
                IngestJob.status.in_(['pending', 'processing'])
            ).count()
            ...
            audit_events_24h = db.query(AuditLog).filter(
                AuditLog.created_at >= last_24h
            ).count()
            
            security_events_24h = db.query(AuditLog).filter(
                AuditLog.created_at >= last_24h,
                AuditLog.severity.in_(['high', 'critical'])
            ).count()
            
            # Average processing time
            completed_jobs_with_time = db.query(IngestJob).filter(
            ...
            return {
                ...
                'knowledge_base': {
                    ...
                    'sensitivity_percentage': round(
                        (sensitive_chunks / total_chunks * 100) if total_chunks > 0 else 0, 2
                    )
                },
                'security': {
