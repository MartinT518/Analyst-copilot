...
class RetrainingService:
    ...
    async def _fetch_feedback_data(
        ...
        query = self.db.query(Feedback).filter(
            Feedback.agent_name == job_config.agent_name,
    ...
    async def _prepare_data(self, feedback_data: List[Feedback]):
        ...
        df = pd.DataFrame([{
            "text": f.original_output,
            "label": 1 if f.accepted else 0
    ...
    async def _tune_model(self, X_train, y_train):
        ...
        def build_model(hp):
            model = Sequential([
                Embedding(input_dim=10000, output_dim=hp.Int("embedding_dim", 16, 128, step=16)),
