...
class MonitoringService:
    ...
    def __init__(self):
        ...
        if self.settings.grafana_url and self.settings.grafana_api_key:
            self.grafana_client = GrafanaApi.from_url(
                ...
                credential=self.settings.grafana_api_key
            )
        
        # Prometheus metrics
        self.edit_rate = Gauge(
            "acp_edit_rate",
            "Rate of edits to agent-generated content",
            ["agent_name"]
        )
        self.task_acceptance_rate = Gauge(
            "acp_task_acceptance_rate",
            "Rate of acceptance for generated tasks",
            ["agent_name"]
        )
    ...
    async def update_monitoring_metrics(
        ...
        try:
            ...
            if "task_acceptance_rate" in metrics_data:
                self.task_acceptance_rate.labels(agent_name=agent_name).set(metrics_data["task_acceptance_rate"])
    ...
    async def create_grafana_dashboard(
        ...
        try:
            # Create dashboard
            dashboard = self.grafana_client.dashboard.update_dashboard(
